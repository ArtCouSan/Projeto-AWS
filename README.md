...
It also uses the DynamoDBMapper ORM structure to map Trips items in a DynamoDB table to a RESTful API for managing Studies.
...
3. Start the SAM local API.
 - On a Mac: `sam local start-api --env-vars src/test/java/test_environment_mac.json`
 - On Windows: `sam local start-api --env-vars src/test/java/test_environment_windows.json`
 - On Linux: `sam local start-api --env-vars src/test/java/test_environment_linux.json`
...
invoke the functions rooted at `http://localhost:3000/trip?starts=2020-01-02&ends=2020-02-02`.
